...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_game_start(self, config):
        ...
        self.start_points = [[2, 12], [5, 12], [8, 12], [11, 12], [
            14, 12], [17, 12], [20, 12], [23, 12], [25, 12]]
    ...
    def on_turn(self, turn_state):
        ...
        for _ in range(max_improvements):
            if state.get_resource(SP) < 2:
                gamelib.debug_write(
                    f"Turn {state.turn_number}: Not enough SP to improve defense")
                break
            if not self._try_improve_funnel_defense(state):
                gamelib.debug_write(
                    f"Turn {state.turn_number}: Failed to improve funnel defense")
                if state.get_resource(SP) < 4:
                    gamelib.debug_write(
                        f"Turn {state.turn_number}: Not enough SP to manage support funnel")
                    break
                else:
                    self._manage_support_funnel(state)
                    gamelib.debug_write(
                        f"Turn {state.turn_number}: Managing support funnel")
                break

        if state.get_resource(SP) >= 15:
            while state.get_resource(SP) >= 4:
                self._manage_support_funnel(state)
    ...
    def on_action_frame(self, turn_str):
        ...
        gamelib.debug_write(data.get('events',{}))
    ...
    def _initial_defense(self, state: GameState):
        ...
        walls = [[2, 13], [5, 13], [8, 13], [11, 13], [
            14, 13], [17, 13], [20, 13], [23, 13], [25, 13]]
    ...
    def improve_defense(self, state: GameState, sector: int, defense):
        ...
        key_positions = [[4, 13], [24, 13], [6, 13], [22, 13],
                         [8, 13], [19, 13], [12, 13], [16, 13], [14, 13]]
    ...
    def improve_funnel_defense(self, state: GameState):
        ...
        for loc in wall_locations:
            if not state.contains_stationary_unit(loc):
                if state.can_spawn(WALL, loc):
                    gamelib.debug_write(
                        f"Turn {state.turn_number}: Spawning wall at {loc}")
                    state.attempt_spawn(WALL, loc)
                    state.attempt_upgrade(loc)
                    return True
            else:
        ...
        for loc in turret_locations:
            if not state.contains_stationary_unit(loc):
                if state.can_spawn(TURRET, loc):
                    ...
                    state.attempt_upgrade(loc)
                    gamelib.debug_write(
                        f"Turn {state.turn_number}: Spawning turret at {loc}")
                    return True
            else:
                unit = state.contains_stationary_unit(loc)
                if unit.health < 0.25 * unit.max_health:
    ...
    def _manage_support(self, state: GameState, scout_loc):
        ...
        if scout_loc and t >= 5:
            ...
            rng = support_config.get(
                "shieldRange", support_config.get("attackRange", 3))
    ...
    def _manage_support_turret(self, state: GameState):
        wall_locations = self._get_funnel_locations()[0]
        wall_locations = wall_locations

        flip = -1 if state.turn_number % 2 else 1

        if state.turn_number % 3:
            for loc in wall_locations[::flip]:
                loc = [loc[0], loc[1]-1]
                if not state.contains_stationary_unit(loc) and state.can_spawn(TURRET, loc):
                    state.attempt_spawn(TURRET, loc)
                    return True
    ...
    def _manage_support_funnel(self, state: GameState):
        ...
        while current_col <= max_col:
            ...
            # upgrade row
            for i in reversed(range(max_units_per_row)):
                loc = [current_col + i, current_row]
                if state.attempt_upgrade(loc):
                    return True
    ...
    def _get_funnel_locations(self):
        ...
        wall_locations = [[0, 13], [1, 13], [2, 13], [5, 9], [5, 13], [7, 11],
                          [7, 12], [8, 10], [9, 9], [10, 8], [11, 8], [12, 8], [13, 8], [14, 8], [
            15, 8], [16, 8], [17, 8], [18, 8], [19, 8], [20, 8], [21, 8], [22, 9], [23, 10], [24, 11], [25, 12], [25, 13], [26, 13], [27, 13]]
    ...
    def attack_1(self, state: GameState):
        ...
        # some kind of failure in funnel strategy
        # if taking significant damage from enemy, use demolisher
        if state.turn_number > 5 and self.breach_on_turn >= 5:
            max_demolishers = int(state.get_resource(MP) // 3)
            for _ in range(max_demolishers):
                if state.can_spawn(DEMOLISHER, demolisher_spawn):
                    state.attempt_spawn(DEMOLISHER, demolisher_spawn)
        ...
        while idx < len(path):
            ...
            while rem > 0:
                ...
                if tgt.health <= maxd:
                    ...
                    if tgt.unit_type == TURRET:
                        dmg_turret += tgt.health
                    elif tgt.unit_type == WALL:
                        dmg_wall += tgt.health
                    elif tgt.unit_type == SUPPORT:
                        dmg_support += tgt.health
    ...
    def column_sequence(self, c):
        ...
        block = (c // 7) * 7
        rng = range(block + 1, block +
                    7) if c < 14 else range(block, block + 6)
        seq = [c]
        for i in range(1, 7):
            for nc in (c - i, c + i):
                if nc in rng:
    ...
    def _update_resource_evolution(self, state: GameState):
        ...
        self._player_resources_evolution[0]['SP'].append(
            p1_resources[0])  # Our SP
        self._player_resources_evolution[0]['MP'].append(
            p1_resources[1])  # Our MP
        self._player_resources_evolution[1]['SP'].append(
            p2_resources[0])  # Enemy SP
        self._player_resources_evolution[1]['MP'].append(
            p2_resources[1])  # Enemy MP

        # Debug write the current state
        if state.turn_number > 0:
            gamelib.debug_write(
                f"Turn {state.turn_number} Resource Evolution:")
            gamelib.debug_write(
                f"Enemy SP: {p2_resources[0]}, MP: {p2_resources[1]}")
            gamelib.debug_write(f"Enemy SP Trend: {
                                self._get_resource_trend(1, 'SP')}")
            gamelib.debug_write(f"Enemy MP Trend: {
                                self._get_resource_trend(1, 'MP')}")
